...
SoftwareSerial HAT(8, 7); // RX, TX pinleri tanımlanıyor.
byte Paket = 0; // Byte türünden alınan veriyi taşıyacak değişken tanımlanıyor.
...
void setup() {
 ...
 // pinMode(A8, INPUT_PULLUP); 
  ...
  Serial.begin(9600);  
  /////////////////////////
  HAT.begin(9600); // İki Arduino arasındaki iletişim başlatılıyor.
  ////////////////////////
...
//////////////////////
 if (HAT.available()) { // Gönderici Arduino'dan veri geldiyse,

    Paket = HAT.read(); // Gelen veri Paket değişkenine aktarılıyor.
       if (Paket == 1) { // Gelen veri beklediğimiz sayı ise,
      RButton1 = 1;
     /* digitalWrite(Led, HIGH); // 1 saniyeliğine led yakılıyor.
      delay(2000);
...
/////////////////////
   Serial.print("Paket___");
    Serial.println(Paket); // Paket değişkeni seri monitöre yazdırılıyor.
    Serial.print("RButton1___"); 
    Serial.println(RButton1); 

// Read Switches
int currentButtonState0 = RButton1; // Button 1 digitalRead(2)
...
int currentButtonState1 = !digitalRead(5); // button 2
